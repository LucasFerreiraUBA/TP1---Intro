function search_registers() {
    const num_registers = document.getElementById("num_registers").value
    const tolerancy_out = document.getElementById("tolerancy-out").value
    const tolerancy_in = document.getElementById("tolerancy-in").value
    document.getElementById("registers_list").innerHTML = ""
    fetch(`http://localhost:5000/api/v1/registers/unpunctual?limit=${num_registers}&tolerancy_in=${tolerancy_in}&tolerancy_out=${tolerancy_out}`)
    .then((res) => res.json())
    .then(handle_data)
...
function delete_register(id) {
    ...
    if (confirm_popup) {
        fetch(`http://localhost:5000/api/v1/registers/${id}`, {method:"DELETE"})
        .then((res) => res.json())
...
function handle_data(data) {
    ...
    for (const register of data) {
        ...
        delete_button.setAttribute("onclick", `delete_register(${register.id})`)
        ...
        let pill_class;
        register.is_check_in ? pill_class = "check_in" : pill_class = "check_out";
...
fetch("http://localhost:5000/api/v1/registers/unpunctual")
.then((res) => res.json())
.then(handle_data)
.catch((error) => console.log("error:", error))